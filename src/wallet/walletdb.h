...
public:
    ...
    bool WriteZWCCGSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZWCCGSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZWCCGSeed_deprecated(uint256& seed);
    bool EraseZWCCGSeed();
    bool EraseZWCCGSeed_deprecated();

    bool WriteZWCCGCount(const uint32_t& nCount);
    bool ReadZWCCGCount(uint32_t& nCount);
