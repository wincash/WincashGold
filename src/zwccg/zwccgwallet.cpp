...
CzWCCGWallet::CzWCCGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zwccg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZWCCGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZWCCGSeed_deprecated()) {
                    LogPrintf("%s: Updated zWCCG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zwccg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zwccg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzWCCGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZWCCGCount(nCountLastUsed);
    else if (!walletdb.ReadZWCCGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzWCCGWallet::Lock()
{
    seedMaster = 0;
}

void CzWCCGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzWCCGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZWCCG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzWCCGWallet::LoadMintPoolFromDB()
{
...
void CzWCCGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzWCCGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzWCCGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zwccgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzWCCGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZWCCG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zwccgTracker which also adds to database
    pwalletMain->zwccgTracker->Add(dMint, true);
...
void CzWCCGWallet::SeedToZWCCG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzWCCGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzWCCGWallet::GenerateDeterministicZWCCG(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzWCCGWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzWCCGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzWCCGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
