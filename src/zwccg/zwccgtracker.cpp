// Copyright (c) 2018-2019 The WINCASHGOLD developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zwccg/deterministicmint.h>
#include "zwccgtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzWCCGTracker::CzWCCGTracker(std::string strWalletFile)
{
...
CzWCCGTracker::~CzWCCGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzWCCGTracker::Init()
{
...
bool CzWCCGTracker::Archive(CMintMeta& meta)
{
...
bool CzWCCGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzWCCGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzWCCGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzWCCGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzWCCGTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzWCCGTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzWCCGTracker::GetSerialHashes()
{
...
CAmount CzWCCGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzWCCGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzWCCGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzWCCGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzWCCGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzWCCGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzWCCGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzWCCGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzWCCGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzWCCGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzWCCGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzWCCGWallet* zWCCGWallet)
{
    bool iszWCCGWalletInitialized = (NULL != zWCCGWallet);
    CMintMeta meta;
    ...
    if (! iszWCCGWalletInitialized)
        zWCCGWallet = new CzWCCGWallet(strWalletFile);
    meta.isSeedCorrect = zWCCGWallet->CheckSeed(dMint);
    if (! iszWCCGWalletInitialized)
        delete zWCCGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzWCCGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzWCCGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzWCCGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzWCCGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzWCCGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzWCCGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzWCCGWallet* zWCCGWallet = new CzWCCGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zWCCGWallet);
        }
        delete zWCCGWallet;
        LogPrint("zero", "%s: added %d dzwccg from DB\n", __func__, listDeterministicDB.size());
    }
