...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "wincashgold" is a composite category enabling all WINCASHGOLD-related debug output
            if (ptrCategory->count(std::string("wincashgold"))) {
...
#else
    const char* pszModule = "wincashgold";
#endif
    if (pex)
        return strprintf(
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\WINCASHGOLD
// Windows >= Vista: C:\Users\Username\AppData\Roaming\WINCASHGOLD
// Mac: ~/Library/Application Support/WINCASHGOLD
// Unix: ~/.wincashgold
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WINCASHGOLD";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "wincashgold.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty wincashgold.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override wincashgold.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "wincashgoldd.pid"));
