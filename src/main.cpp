...
#include "zwccg/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "WINCASHGOLD cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uwccg) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend->getCoinSerialNumber(), nHeightTx))
        return error("%s : zWCCG spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zWCCG is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zWCCG spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zWCCG without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zWCCG v1 serial spend not spendable, serial %s, tx %s\n", __func__,
    ...
    if (!spend->HasValidSerial(Params().Zerocoin_Params(v1Serial)))  {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zWCCG spend with serial %s from tx %s is not in valid range\n", __func__,
                     spend->getCoinSerialNumber().GetHex(), tx.GetHash().GetHex());
        else
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZWCCGModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZWCCGModule::validateInput(txin, prevOut, tx, ret)){
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zWCCG spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!isPrivZerocoinSpend && !isPublicSpend) {
                    return state.Invalid(error("%s: AcceptToMemoryPool failed for tx %s, every input must be a zcspend or zcpublicspend", __func__,
                                        tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zwccg");
                }
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZWCCGModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
            ...
            // Check that zWCCG mints (if included) are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
    if (nHeight == 0) {
        nSubsidy = 60000 * COIN; // premine amount
    } else if (nHeight >= 1 && nHeight <= 576){
        nSubsidy = 1 * COIN;
    } else if (nHeight >= 577 && nHeight <= 17280) {
        nSubsidy = 1 * COIN;
    } else if (nHeight >= 17281 && nHeight <= 34560) {
        nSubsidy = 1.1 * COIN;
    } else if (nHeight >= 34561 && nHeight <= 51840) {
        nSubsidy = 1.2 * COIN;
    } else if (nHeight >= 51841 && nHeight <= 69120) {
        nSubsidy = 1.3 * COIN;
    } else if (nHeight >= 69121 && nHeight <= 86400) {
        nSubsidy = 1.4 * COIN;
    } else if (nHeight >= 86401 && nHeight <= 103680) {
        nSubsidy = 1.5 * COIN;
    } else if (nHeight >= 103681 && nHeight <= 120960) {
        nSubsidy = 1.6 * COIN;
    } else if (nHeight >= 120961 && nHeight <= 138240) {
        nSubsidy = 1.7 * COIN;
    } else if (nHeight >= 138241 && nHeight <= 155520) {
        nSubsidy = 1.8 * COIN;
    } else if (nHeight >= 155521 && nHeight <= 172800) {
        nSubsidy = 1.9 * COIN;
    } else if (nHeight >= 172801 && nHeight <= 190080) {
        nSubsidy = 2.0 * COIN;
    } else if (nHeight >= 190081 && nHeight <= 241920) {
        nSubsidy = 2.1 * COIN;
    } else if (nHeight >= 241921 && nHeight <= 293760) {
        nSubsidy = 2.3 * COIN;
    } else if (nHeight >= 293761 && nHeight <= 345600) {
        nSubsidy = 2.6 * COIN;
    } else if (nHeight >= 345601 && nHeight <= 397440) {
        nSubsidy = 2.9 * COIN;
    } else if (nHeight >= 397441 && nHeight <= 501120) {
        nSubsidy = 3.2 * COIN;
    } else if (nHeight >= 501121 && nHeight <= 604800) {
        nSubsidy = 3.8 * COIN;
    } else if (nHeight >= 604801) {
        nSubsidy = 4.6 * COIN;
    }
    return nSubsidy;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZWCCGStake)
{
    int64_t ret = 0;
    if (nHeight >= Params().LAST_POW_BLOCK()) {
        return blockValue * 99994 / 100000;
    }
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZWCCGModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from WINCASHGOLD
...
{
    RenameThread("wincashgold-scriptch");
...
void RecalculateZWCCGMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted ZWCCG..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted ZWCCG..."), percent);
        }
...
void RecalculateZWCCGSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent ZWCCG..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent ZWCCG..."), percent);
        }

        //Rewrite zWCCG supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zWCCG supply
...
bool RecalculateWCCGSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating WCCG supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating WCCG supply..."), percent);
        }
...
{
    // WINCASHGOLD: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZWCCGSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zWCCG supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            // Check that zWCCG mints are not already known
            if (tx.HasZerocoinMintOutputs()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZWCCGMinted();
        RecalculateZWCCGSpent();
        RecalculateWCCGSupply(Params().Zerocoin_StartHeight());
    }

    //Track zWCCG money supply in the block index
    if (!UpdateZWCCGSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zWCCG supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zWccgSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
     ...
     *
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert WCCG to zWCCG
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zWCCG spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zWCCG serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // WincashGold Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().WincashGoldBadBlockTime()) && (block.nBits == (uint32_t) Params().WincashGoldBadBlocknBits())) {
            // accept WINCASHGOLD block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> wccgInputs;
        std::vector<CTxIn> zWCCGInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zWCCGInputs.push_back(stakeIn);
            }else{
                wccgInputs.push_back(stakeIn);
            }
        }
        const bool hasWCCGInputs = !wccgInputs.empty();
        const bool hasZWCCGInputs = !zWCCGInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasWCCGInputs)
                    ...
                    for (const CTxIn& wccgIn : wccgInputs){
                        if(wccgIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : wccgInputs) {
            ...
            // Now that this loop if completed. Check if we have zWCCG inputs.
            if(hasZWCCGInputs){
                for (const CTxIn& zWccgInput : zWCCGInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zWccgInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zwccg");
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // WINCASHGOLD: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zwccg", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zwccg", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
