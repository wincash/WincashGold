...
#include "ui_zwccgcontroldialog.h"
...
std::set<std::string> ZWccgControlDialog::setSelectedMints;
std::set<CMintMeta> ZWccgControlDialog::setMints;

bool CZWccgControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZWccgControlDialog::COLUMN_DENOMINATION || column == ZWccgControlDialog::COLUMN_VERSION || column == ZWccgControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZWccgControlDialog::ZWccgControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZWccgControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZWccgControlDialog::~ZWccgControlDialog()
{
    delete ui;
}

void ZWccgControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWccgControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZWccgControlWidgetItem* itemDenom(new CZWccgControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZWccgControlWidgetItem *itemMint = new CZWccgControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zwccgTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zwccgTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zWCCG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zWCCG seed used to mint this zWCCG is not the same as currently hold in the wallet";
            else
...
void ZWccgControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWccgControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZWccg_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWccgControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZWccgControlDialog::GetSelectedMints()
{
...
void ZWccgControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
