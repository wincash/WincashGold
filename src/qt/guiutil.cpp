...
// Copyright (c) 2015-2018 The WINCASHGOLD developers
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a WINCASHGOLD address (e.g. %1)").arg("Wacm6T7Bn8v9xorcz72odSn4CBRdCXn2LN"));
...
{
    // return if URI is not valid or is no WINCASHGOLD: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::WCCG, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert wincashgold:// to wincashgold:
    //
    //    Cannot handle this later, because wincashgold:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::WCCG, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open wincashgold.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "WINCASHGOLD.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for WINCASHGOLD.lnk
...
{
    return GetAutostartDir() / "wincashgold.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a wincashgold.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=WINCASHGOLD\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the wincashgold app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add wincashgold app to startup item list
