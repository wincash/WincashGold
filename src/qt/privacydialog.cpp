// Copyright (c) 2017-2019 The WINCASHGOLD developers
                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zWCCG ought to be enough for anybody." - Bill Gates, 2017
    ui->zWCCGpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzWCCGSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zWCCGpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzWCCG_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zWCCG is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zWCCG, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zWCCG...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zWCCG in ") +
...
void PrivacyDialog::on_pushButtonSpendzWCCG_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zWCCG is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zWCCG, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zWCCG
        sendzWCCG();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zWCCG
    sendzWCCG();
}

void PrivacyDialog::on_pushButtonZWccgControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZWccgControlDialog* zWccgControl = new ZWccgControlDialog(this);
    zWccgControl->setModel(walletModel);
    zWccgControl->exec();
}

void PrivacyDialog::setZWccgControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzWccgSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzWCCG()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid WincashGold Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zWCCGpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zWCCG is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " WCCG </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zWCCG</b>";
    ...
    // use mints from zWCCG selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZWccgControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZWccgControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zWCCG transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zWCCG was spent successfully update the addressbook with the label
    ...
    // Clear zwccg selector in case it was used
    ZWccgControlDialog::setSelectedMints.clear();
    ui->labelzWccgSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zWCCG Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Wccg, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zWCCG Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zwccgTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zWCCG </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zWCCG "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zWCCG "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zWCCG "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zWCCG </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zWCCG </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zWCCG </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzWCCGSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzWCCG->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zWCCG
        //ui->pushButtonMintzWCCG->setEnabled(false);
        //ui->pushButtonMintzWCCG->setToolTip(tr("zWCCG is currently disabled due to maintenance."));

        // Spend zWCCG
        ui->pushButtonSpendzWCCG->setEnabled(false);
        ui->pushButtonSpendzWCCG->setToolTip(tr("zWCCG is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zWCCG
        //ui->pushButtonMintzWCCG->setEnabled(true);
        //ui->pushButtonMintzWCCG->setToolTip(tr("PrivacyDialog", "Enter an amount of WCCG to convert to zWCCG", 0));

        // Spend zWCCG
        ui->pushButtonSpendzWCCG->setEnabled(true);
        ui->pushButtonSpendzWCCG->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
