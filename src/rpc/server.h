...
/**
 * WINCASHGOLD RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZwccgSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzwccgseed(const UniValue& params, bool fHelp);
extern UniValue getzwccgseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzwccg(const UniValue& params, bool fHelp);
extern UniValue dzwccgstate(const UniValue& params, bool fHelp);
